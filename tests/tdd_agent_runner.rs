...
fn a_proceeding_agent_makes_the_step_succeed() {
    ...
    assert_eq!(handler.handle(&Step::abort("plan"), &Blackboard::new()).outcome, StepOutcome::Success);
}
...
fn a_rejecting_agent_makes_the_step_fail() {
    ...
    assert_eq!(handler.handle(&Step::abort("critic"), &Blackboard::new()).outcome, StepOutcome::Failure);
}
...
fn a_failing_agent_makes_the_step_fail() {
    ...
    assert_eq!(handler.handle(&Step::abort("plan"), &Blackboard::new()).outcome, StepOutcome::Failure);
}
...
fn the_agent_receives_the_step_role() {
    ...
    handler.handle(&Step::abort("reviewer"), &Blackboard::new());
